<div class="containerpric">
    <x-loading functionsList="addDeadtime, removeDeadtime, updateProcessData, finishProcess" />
    ...
    <div class="bg-white rounded-lg shadow-lg my-3 p-4 lg:space-y-10">
        ...
        <div class="flex flex-col space-y-2 mt-5">
            <p class="font-semibold text-primarycolor">Agregar tiempo muerto</p>
            <div class="w-full lg:w-1/2 flex space-x-3">
                <div class="sm:w-1/3">
                        <p class="text-secondarycolor">Razón</p>
                        <select wire:model="deadtime" id="deadtime_select" class="inputcatalogues w-full">
                            <option value="">Seleccionar...</option>
                    ...
                    <div class="sm:w-1/3">
                            <p class="text-secondarycolor">Tiempo en horas:</p>
                            <div class="flex flex-col sm:flex-row sm:items-end sm:space-x-2">
                                <div class="flex-1 sm:min-w-[10rem]">
                                    <input wire:model="deadtime_hours" type="number" class="inputcatalogues w-full">
                                    <span class="text-red-500 text-xs italic">
                                        @error('deadtime_hours')
                                            {{$message}}
                                        @enderror
                                    </span>
                                </div>
            ...
            @if(count($deadTimesList) > 0)
                    <div class="w-full lg:w-1/2 mt-4">
                        <p class="font-semibold text-primarycolor">Tiempos muertos registrados</p>
